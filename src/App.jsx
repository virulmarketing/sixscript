...
export default function StrikeScript() {
  ...
  const savePlanToDate = async () => { if(!practiceDate) return; const inlinePlan={inline:true,label:practiceDate,startH,startM,startAP,endH,endM,endAP,notes:planNotes,segments:serializeSegments(segments)}; const cal={...calendarPlans,[practiceDate]:inlinePlan}; setCalendarPlans(cal); await authFetch('/api/plans/calendar-save', { dateKey: practiceDate, planValue: inlinePlan }); setPracticeDate(null); setView("calendar"); };
  ...
  const assignPlanToDate = async (dk, pid) => { const n={...calendarPlans,[dk]:pid}; setCalendarPlans(n); await authFetch('/api/plans/calendar-save', { dateKey: dk, planValue: pid }); setShowAssignPlan(null); };
  const removePlanFromDate = async dk => { const n={...calendarPlans}; delete n[dk]; setCalendarPlans(n); await authFetch('/api/plans/calendar-remove', { dateKey: dk }); };
  const addSegment = t => { const uid="_"+Date.now(); if(t.splitType==="unit"){const groups=config.unitGroups||[];setSegments([...segments,{...t,_uid:uid,duration:t.defaultDur||20,drills:[],tracks:groups.map(g=>({...g,drills:[]}))}]);} else if(t.splitType==="position"){const groups=(footballType==="flag"?config.flagPositionGroups:config.positionGroups)||[];setSegments([...segments,{...t,_uid:uid,duration:t.defaultDur||30,drills:[],tracks:groups.map(g=>({...g,drills:[]}))}]);} else {setSegments([...segments,{...t,_uid:uid,duration:0,drills:[]}]);} };
