...
export default function StrikeScript() {
  ...
  const usedMin = segments.reduce((s,seg)=>s+seg.drills.reduce((a,d)=>a+d.allocatedMin,0),0);
  const remainMin = totalMin - usedMin;
  const activeSeg = segments[activeSegIdx];
  const suggestedCats = useMemo(() => activeSeg ? SEGMENT_TEMPLATES.find(t=>t.name===activeSeg.name)?.suggestedCats||activeSeg.suggestedCats||[] : [], [activeSeg]);
  ...
  const savePracticePlan = async () => { if(!savePlanName.trim()) return; const p={id:"pl_"+Date.now(),label:savePlanName.trim(),createdBy:user?.name,startH,startM,startAP,endH,endM,endAP,notes:planNotes,segments:segments.map(s=>({name:s.name,color:s.color,duration:s.duration,suggestedCats:s.suggestedCats||[],drills:s.drills.map(d=>({id:d.id,allocatedMin:d.allocatedMin,coach:d.coach}))}))}; const n=[...savedPlans,p]; setSavedPlans(n); if(getAuthToken()){await apiFetch('/api/plans/save',{plan:p});}else{await sv("sk-sp",n);} setSavePlanName(""); setShowSavePlan(false); };
  const savePlanToDate = async () => { if(!practiceDate) return; const inlinePlan={inline:true,label:practiceDate,startH,startM,startAP,endH,endM,endAP,notes:planNotes,segments:segments.map(s=>({name:s.name,color:s.color,duration:s.duration,suggestedCats:s.suggestedCats||[],drills:s.drills.map(d=>({id:d.id,allocatedMin:d.allocatedMin,coach:d.coach}))}))}; const cal={...calendarPlans,[practiceDate]:inlinePlan}; setCalendarPlans(cal); await sv("sk-cal",cal); setPracticeDate(null); setView("calendar"); };
  const loadPracticePlan = (p, date=undefined) => { setStartH(p.startH);setStartM(p.startM);setStartAP(p.startAP);setEndH(p.endH);setEndM(p.endM);setEndAP(p.endAP);setStartHStr(String(p.startH));setStartMStr(String(p.startM).padStart(2,"0"));setEndHStr(String(p.endH));setEndMStr(String(p.endM).padStart(2,"0")); setSegments(p.segments.map(s=>({...s,drills:s.drills.map(sd=>{const f=allDrills.find(d=>d.id===sd.id);return f?{...f,allocatedMin:sd.allocatedMin,coach:sd.coach||user?.name}:null;}).filter(Boolean)}))); setPlanNotes(p.notes||""); if(date!==undefined) setPracticeDate(date); setStep(3); setView("planner"); };
  ...
  const addSegment = t => { setSegments([...segments,{...t,duration:0,drills:[]}]); };
  ...
  const autoAllocate = idx => { const s=[...segments]; const seg=s[idx]; if(!seg.drills.length) return; s[idx]={...seg,drills:seg.drills.map(d=>({...d,allocatedMin:d.dur}))}; setSegments(s); };
  ...
  if (step === 1) return (
    <div style={S.app}><Header/>
      ...
      <div style={S.body}>
        ...
        {segments.length>0&&<div style={S.card}><span style={S.label}>Your Segments</span>{segments.map((seg,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:B.surface,borderRadius:8,marginBottom:4,borderLeft:`3px solid ${seg.color}`}}><div style={{display:"flex",flexDirection:"column",gap:2}}><button onClick={()=>moveSegment(i,i-1)} style={{background:"none",border:"none",color:B.textDim,cursor:"pointer",fontSize:9,padding:0,lineHeight:1}}>▲</button><button onClick={()=>moveSegment(i,i+1)} style={{background:"none",border:"none",color:B.textDim,cursor:"pointer",fontSize:9,padding:0,lineHeight:1}}>▼</button></div><div style={{flex:1}}><div style={{fontWeight:700,fontSize:13,color:B.black}}>{seg.name}</div>{seg.drills.length>0&&<div style={{fontSize:10,color:B.textDim,marginTop:1}}>{seg.drills.length} drill{seg.drills.length!==1?"s":""}</div>}</div><button onClick={()=>removeSegment(i)} style={{background:"none",border:"none",color:B.danger,cursor:"pointer",fontSize:14,padding:"0 4px",opacity:0.5}}>×</button></div>))}</div>}
        ...
        <div style={S.card}><span style={S.label}>Add Segments</span><div style={{display:"flex",flexWrap:"wrap",gap:mob?8:10}}>{SEGMENT_TEMPLATES.filter(t=>!segments.find(s=>s.name===t.name)).map(t=><button key={t.name} onClick={()=>addSegment(t)} style={S.segChip(t.color,false)}>{t.name}</button>)}</div></div>
  ...
  // ═══════════ STEP 2: DRILLS ═══════════
  if (step === 2) {
    const sdt = activeSeg?.drills.reduce((a,d)=>a+d.allocatedMin,0)||0;
    return (
      <><div style={S.app}><Header/>
        <div style={S.body}>
          <div style={{display:"flex",gap:mob?8:10,marginBottom:24,overflowX:"auto",paddingBottom:6}}>{segments.map((seg,i)=><button key={i} ref={el=>segBtnRefs.current[i]=el} onClick={()=>{setActiveSegIdx(i);setSearchQ("");setFilterCat("suggested");}} style={{...S.segChip(seg.color,i===activeSegIdx),whiteSpace:"nowrap",flexShrink:0}}>{seg.name}{seg.drills.length>0&&<span style={{fontSize:mob?10:12,opacity:0.5,marginLeft:4}}>({seg.drills.length})</span>}</button>)}</div>
          <div style={{display:mob?"flex":"grid",flexDirection:"column",gridTemplateColumns:mob?undefined:"1fr 380px",gap:mob?12:20}}>
            <div><div style={{...S.card,padding:16}}>
              ...
              <div style={{maxHeight:460,overflowY:"auto"}}>
                {filteredDrills.map(d=>{const added=activeSeg?.drills.find(x=>x.id===d.id);const cat=CATEGORIES.find(c=>c.id===d.cat);const isFav=favorites.has(d.id);
                  return(<div key={d.id} style={{background:added?"#F0FAF0":B.surface,border:`1px solid ${added?B.success+"30":B.cardBorder}`,borderRadius:10,padding:"11px 12px",marginBottom:6,cursor:"pointer"}}>
                    <div style={{display:"flex",alignItems:"start",gap:6}}>
                      ...
                      <div style={{flex:1}} onClick={()=>!added&&addDrillToSegment(d)}>
            ...
            <div><div style={S.card}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:6}}>
                <span style={{...S.label,marginBottom:0}}>{activeSeg?.name} — {activeSeg?.duration}m</span>
                <div style={{display:"flex",gap:6}}>
                  {activeSeg?.drills.length>0&&<button style={{...S.btn(false),fontSize:9,padding:"4px 10px"}} onClick={()=>autoAllocate(activeSegIdx)}>Auto-Fill</button>}
                  {activeSeg?.drills.length>0&&<button style={{...S.btn(false),fontSize:9,padding:"4px 10px",color:B.red,borderColor:B.redMed}} onClick={()=>{setSaveSegName(activeSeg.name);setShowSaveSegment(true);}}>Save</button>}
                </div>
              </div>
              <div style={{height:5,background:B.surface,borderRadius:3,marginBottom:12,overflow:"hidden"}}><div style={{width:`${Math.min(100,(sdt/(activeSeg?.duration||1))*100)}%`,height:"100%",background:sdt>activeSeg?.duration?B.danger:activeSeg?.color||B.red,borderRadius:3}}/></div>
              <div style={{fontSize:10,color:sdt>activeSeg?.duration?B.danger:B.textDim,marginBottom:14,fontWeight:600}}>{sdt}m / {activeSeg?.duration}m</div>
              {!activeSeg?.drills.length&&<div style={{color:B.textDim,fontSize:12,textAlign:"center",padding:28}}>Select drills from the left</div>}
              {activeSeg?.drills.map(d=>{const tO=getTimeOptions(d.dur);const shC=customTimeId===d.id;const isC=!tO.includes(d.allocatedMin);
                return(<div key={d.id} style={{padding:"10px 12px",background:B.surface,borderRadius:8,marginBottom:4,borderLeft:`3px solid ${activeSeg.color}`}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div style={{fontWeight:700,fontSize:12,color:B.black,flex:1}}>{d.name}</div>
                    <button onClick={()=>removeDrillFromSegment(d.id)} style={{background:"none",border:"none",color:B.danger,cursor:"pointer",fontSize:13,padding:"0 4px",opacity:0.4}}>×</button>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginTop:6}}>
                    <span style={{fontSize:9,color:B.textDim,fontWeight:700,textTransform:"uppercase",letterSpacing:"1px"}}>Coach:</span>
                    <select value={d.coach||user?.name} onChange={e=>updateDrillCoach(d.id,e.target.value)} style={S.coachSelect}>
                      {teamCoaches.map(c=><option key={c.email} value={c.name}>{c.name}</option>)}
                    </select>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:4,marginTop:8,flexWrap:"wrap"}}>
                    {tO.map(t=><button key={t} onClick={()=>{updateDrillTime(d.id,t);setCustomTimeId(null);}} style={S.speedBtn(d.allocatedMin===t,activeSeg.color)}>{t}m</button>)}
                    {shC?<input autoFocus style={S.customTimeInput} placeholder="#" value={customTimeVal} onChange={e=>setCustomTimeVal(e.target.value.replace(/\D/g,""))} onKeyDown={e=>{if(e.key==="Enter"&&customTimeVal){updateDrillTime(d.id,Math.max(1,parseInt(customTimeVal)));setCustomTimeId(null);setCustomTimeVal("");}}} onBlur={()=>{if(customTimeVal)updateDrillTime(d.id,Math.max(1,parseInt(customTimeVal)));setCustomTimeId(null);setCustomTimeVal("");}}/>
                    :<button onClick={()=>{setCustomTimeId(d.id);setCustomTimeVal(isC?String(d.allocatedMin):"");}} style={{...S.speedBtn(isC,activeSeg.color),borderStyle:isC?"solid":"dashed",fontSize:10,padding:"4px 8px"}}>{isC?`${d.allocatedMin}m`:"..."}</button>}
                  </div>
                </div>);
              })}
            </div></div>
  ...
  return (
    <><div style={S.app}>
      ...
      <div style={{background:B.black,padding:mob?"16px 12px":"24px 32px"}}><div style={{display:"flex",gap:mob?16:32,justifyContent:"center",flexWrap:"wrap"}}>{[{v:totalMin+"m",l:"Total"},{v:segments.length,l:"Seg"},{v:plan.filter(p=>!p.drillName.includes("Coach")).length,l:"Drills"},{v:teamCoaches.length,l:"Staff"}].map(s=><div key={s.l} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2}}><div style={{fontSize:mob?20:28,fontWeight:800,color:B.white,lineHeight:1}}>{s.v}</div><div style={{fontSize:mob?8:9,color:B.red,fontWeight:700,textTransform:"uppercase",letterSpacing:"1.5px"}}>{s.l}</div></div>)}</div></div>
      <div style={S.body}>
        ...
        <div style={S.card}>
          ...
          {plan.map((row,i)=>{const sh=row.segName!==curSeg;curSeg=row.segName;
            return(<div key={i}>
